...
namespace Algorithm
{
	namespace Graph
	{
		...
		)
		{
			...
			while (!_queue.empty())
			{
				UINT _queueSize = static_cast<UINT>(_queue.size());		// キューサイズの更新
				a_outGroupVec.push_back({});							// グループの作成
				for (UINT _i = 0; _i < _queueSize; ++_i)
				{
					...
					_queue.pop();
					a_outGroupVec.back().push_back(a_nodeVec[_idx].get());
			...
			UINT _nodeSize = 0;
			for (auto& _group : a_outGroupVec)
			{
				_nodeSize += _group.size();
			}
