...
namespace App::Game
{
	UINT GameFlowStateMachine::GetStateHash(const std::string& a_stateName) const
	{
	...
	void GameFlowStateMachine::Load(const std::string& a_filePath)
	{
		...
		Engine::Persistence::Archive _arch(
			Engine::Persistence::Archive::Mode::Load, _dir, _fileName, "stet", Engine::Persistence::Archive::ArchiveFormat::Json);
		...
		if (_arch.BeginArray("TransitionArrows", _arrowMapSize))
		{
			for (size_t _i = 0; _i < _arrowMapSize; ++_i)
			{
				if (_arch.BeginObject(_i))
				{
					...
					size_t _arrowSize = 0;
					std::vector<FlowTransitionArrow> _arrowVec;
	...
	const FlowStateNode* GameFlowStateMachine::GetStateNode(UINT a_stateHash) const
	{
	...
	{
		// デフォルトが設定されていなかった場合のため
		for (auto& [_hash, _node] : m_stateNodeMap)
		{
			if (_node.sceneGUID == m_farstSceneGUID)
			{
				m_defaultStartHash = _hash;
			}
		}
		m_currentStateHash = m_defaultStartHash;
	...
	bool GameFlowStateMachine::Evaluate(Engine::GUID& out_nextSceneGUID)
	{
		...
		auto _it = m_transitionArrowMap.find(m_currentStateHash);
		if (_it == m_transitionArrowMap.end())
		{
			return false; // 遷移先がない場合は現状維持
		}
		...
		for (const auto& _arrow : _it->second)
		{
			...
			for (const auto& _cond : _arrow.conditions)
			{
				...
				{
					if (m_currentFloatParams.find(_cond.paramHash) == m_currentFloatParams.end())
					{
						m_currentFloatParams[_cond.paramHash] = _paramDef.defaultFloat;
					}

					float _val = m_currentFloatParams[_cond.paramHash];
				...
				case EParamType::Int:
				{
					if (m_currentIntParams.find(_cond.paramHash) == m_currentIntParams.end())
					{
						m_currentIntParams[_cond.paramHash] = _paramDef.defaultInt;
					}

					int _val = m_currentIntParams[_cond.paramHash];
				...
				{
					if (m_currentBoolParams.find(_cond.paramHash) == m_currentBoolParams.end())
					{
						m_currentBoolParams[_cond.paramHash] = _paramDef.defaultBool;
					}

					bool _val = m_currentBoolParams[_cond.paramHash];
			...
			{
				// 使用したTriggerパラメーターの消費（フラグを折る）
				for (const auto& _cond : _arrow.conditions)
				{
					if (m_parameters.at(_cond.paramHash).type == EParamType::Trigger)
					{
						m_currentBoolParams[_cond.paramHash] = false;
					}
				}

				// マシン自身の現在ステートを新しいステートに更新
				m_currentStateHash = _arrow.dstStartHash;

				// 遷移先のシーンGUIDを取得して出力変数に詰める
				const auto* _pNextNode = GetStateNode(m_currentStateHash);
				if (_pNextNode)
				{
					out_nextSceneGUID = _pNextNode->sceneGUID;
				}
				else
				{
					// 万が一ノードが存在しない場合のフェイルセーフ
					out_nextSceneGUID = Engine::DefaultGUID;
				}
	...
	void GameFlowStateMachine::AddNode()
	{
		// ノード追加ボタン
	...
	void GameFlowStateMachine::RessetButton()
	{
	...
	void GameFlowStateMachine::CreateArrow()
	{
		// 遷移Arrowの設定
		int _hoveredLinkId;
	...
	void GameFlowStateMachine::ArrowPopUp()
	{
		if (ImGui::BeginPopup("EditConditionPopup"))
		{
			// 選択されたArrowを検索
			FlowTransitionArrow* _pEditArrow = nullptr;
			std::vector<FlowTransitionArrow>* _pArrowVec = nullptr;
			...
			if (ImGui::Button("Delete Arrow", ImVec2(90, 0)))
			{
				...
				auto _it = std::remove_if(_pArrowVec->begin(), _pArrowVec->end(),
					[this](const FlowTransitionArrow& a) { return a.linkID == m_editingLinkID; });
			...
			for (auto _it = _pEditArrow->conditions.begin(); _it != _pEditArrow->conditions.end();)
			{
				...
				Engine::Editor::DrawEnumCombo("Condition", _it->op);
				// 閾値の入力UI
				if (m_parameters.find(_it->paramHash) != m_parameters.end())
				{
	...
	void GameFlowStateMachine::EditParameters()
	{
	...
	void GameFlowStateMachine::EditNode(FlowStateNode& a_node)
	{
		// ノード開始
		ImNodes::BeginNode(a_node.nodeID);

		Engine::Editor::Node::TitleBar(a_node.name);
