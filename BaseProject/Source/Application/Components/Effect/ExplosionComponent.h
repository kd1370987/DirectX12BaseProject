...
// ・死亡演出の入口だった DeathEffectComponent は EffectAsset 専用になったので、
//   ここから出す経路はもう無い。今の出どころはプレハブに直接付けた場合だけ。
//
// ※ EffectAsset に置き換え済み(残してあるのは既存データのため)
//   「時間差で複数のエフェクトを出す」は EffectAsset のパーツごとの
//   EffectTiming(startDelay / duration)で表せるので、新しく作るものは
//   EffectAssetComponent 1つで済む。プレハブを何段も重ねる必要もない。
//
//   このコンポーネントを消すと登録順が詰まってコンポーネントのタイプIDがずれ、
//   保存済みのプレハブ・シーンがすべて壊れるため、消さずに残している。
//
//==========================================================================================
