...
{
	a_world.StartTask<ColliderComponent, ModelComponent,TransformComponent>(
			...
			)
		{
			for (size_t _i = 0; _i < a_count; ++_i)
			{
				...
				if (!_meshs.empty())
				{
					...
					{
						const auto& _meta = _mesh->GetMetaData();
						DirectX::BoundingBox::CreateMerged(_aabb, _aabb, _meta.aabb);
					}
				}
				...
				_inst.worldMat = _mat;
				_inst.worldAABB = _aabb;
				_pCollWorld->AllcateStaticEntity(_inst);
