...
namespace Engine::ECS
{
	...
	public:
		...
		const std::vector<ArchetypeChunk*>& GetArchetypeChunk(const Signature& a_sig);
		...
		std::vector<ArchetypeChunk*> MatchingArchetypeChunkVec(const Signature& a_sig);
		...
		/// <returns>ヒットしたアーキタイプポインタ配列</returns>
		std::vector<ArchetypeChunk*> MatchingArchetypeChunkVecEx(const Signature& a_sig, const Signature& a_excludeSig);
	...
	private:
		...
		ArchetypeChunk* CreateArchetypeChunk(const Signature& a_sig);
		...
		void CalcChunkLayout(ArchetypeChunk* a_pChunk, const Signature& a_sig, size_t a_memorySize);
