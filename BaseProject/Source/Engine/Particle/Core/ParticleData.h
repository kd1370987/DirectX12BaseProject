...
namespace Engine::Particle
{
	...
	// 席を回してよくなるまでの猶予(フレーム)
	//
	// 席の鍵はエンティティなので、シーンを読み直すたびに作り直されるもの
	// (ブースターなど)は毎回ちがう鍵で席を取る。返す仕組みが無いと
	// 数回の読み直しで席が尽きて、そこから先はワールド空間で出てしまう。
	//
	// そこで「このフレーム数のあいだ一度も使われていない席」は空きとみなして回す。
	// 出し終わった粒が消えるまでの猶予でもあるので、
	// パーティクルの寿命より十分に長く取ること(60fps でおよそ2秒)。
	//======================================================================================
	inline constexpr uint64_t EMITTER_SLOT_KEEP_FRAMES = 120;
