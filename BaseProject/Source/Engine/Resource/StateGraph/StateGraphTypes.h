...
// StateGraph 共通型
//
// Animator / ゲームプレイFSM で共通して使う
// 「パラメータ」「遷移条件」「遷移矢印」「ノードのつなぎ情報」「実行時パラメータ」を定義する。
// 各マシン固有のデータ（再生アニメ・アクション設定など）は
...
namespace Engine::StateGraph
{
	...
	// パラメータ定義の確保
	//
	// プログラム側(システム)から書き込むパラメータは、設計図の定義マップに
	// 載っていないとエディターのパラメータ一覧に出てこない。
	// そこで「無ければ定義を追加してから値を入れる」ために、定義側を確保する。
	// 既にある場合は何もせず、その定義を返す(型は編集側の設定を尊重する)。
	//--------------------------------------------------------------------------------------
