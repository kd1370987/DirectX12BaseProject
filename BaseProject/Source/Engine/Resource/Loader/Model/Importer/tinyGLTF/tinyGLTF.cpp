...
)
{
	...
	for (UINT _i = 0; _i < static_cast<UINT>(a_src->nodes.size()); ++_i)
	{
		...
		if (_srcNode.isMesh)
		{
			...
			auto _upMesh = std::make_unique<Engine::Resource::Mesh>();
			...
			_dstNode.meshIndices.push_back(static_cast<int>(a_dst.upMeshVec.size()));
			a_dst.upMeshVec.push_back(std::move(_upMesh));
		}
...
)
{
	...
	a_dst.upMaterialVec.resize(a_src->materials.size());

	for (UINT _i = 0; _i < a_dst.upMaterialVec.size(); ++_i)
	{
		// マテリアル情報
		a_dst.upMaterialVec[_i] = std::make_unique<Engine::Resource::Material>();
		const Engine::Resource::GLTF::Material& _srcMaterial = a_src->materials[_i];	// 入力元
		std::unique_ptr<Engine::Resource::Material>& _dstMaterial = a_dst.upMaterialVec[_i];					// 出力先
...
)
{
	...
	// アニメーション作成
	//=================================================
	a_dst.upAnimationVec.resize(a_src->animations.size());		// アニメーション配列確保

	for (UINT _i = 0; _i < a_dst.upAnimationVec.size(); ++_i)
	{
		auto _srcAnimation = a_src->animations[_i];	// 元データ

		a_dst.upAnimationVec[_i] = std::make_unique<Engine::Resource::AnimationData>();
		std::unique_ptr<Engine::Resource::AnimationData>& _dstAnimation = a_dst.upAnimationVec[_i];							// 出力先
