...
namespace Engine::Resource
{
	...
	public:
		...
		//-------------------------------------------------------------------------------------------------
		// メタファイルを先回りして作り、GUIDを発行する
		//
		// 普通のアセットの新規作成では呼ばないこと。
		// ファイルを置けば Update() の監視が拾ってメタファイルもGUIDも用意する。
		//
		// ここを通すのは「書き出す中身にGUIDが要る」ものだけ。
		//   ・モデル変換 : サブアセット(マテリアル/メッシュ/アニメ)のGUIDを .mdl 本体へ書く
