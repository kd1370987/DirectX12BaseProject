...
namespace Engine::Resource::Parse
{
	...
	struct RawMaterial
	{
		...
		Math::Color		baseColorFactor = { 1,1,1,1 };	// 乗算用
		...
		Math::Vector3	emissiveFactor = { 1,1,1 };		// エミッシブの乗算用
	...
	struct RawNode
	{
		...
		Math::Matrix		localTransform = {};		// ローカル行列	中心点からの座標
		Math::Matrix		worldTransform = {};		// ワールド行列 中心点
		Math::Matrix		inverseBindMatrix = {};		// ボーンのオフセット行列
	...
	struct RawAnimationNode
	{
		...
		// 各チャンネル
		std::vector<Engine::Resource::AnimationKeyVector3>	translations = {};	// 座標キーリスト
		std::vector<Engine::Resource::AnimationKeyQuaternion> rotations = {};	// 回転キーリスト
		std::vector<Engine::Resource::AnimationKeyVector3>	scales = {};		// 拡縮キーリスト
	};
