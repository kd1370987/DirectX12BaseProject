...
std::vector<Engine::Resource::Parse::RawNode> ParseNode(const tinygltf::Model& a_tinyModel)
{
	...
	for (UINT _nodeIdx = 0; _nodeIdx < _result.size(); ++_nodeIdx)
	{
		...
		_destNode->localTransform = Math::Matrix::Identity();
		_destNode->worldTransform = Math::Matrix::Identity();
		_destNode->inverseBindMatrix = Math::Matrix::Identity();
		...
		// 変換行列取得
		//-----------------------
		Math::Matrix _sMat = Math::Matrix::Identity();
		Math::Matrix _rMat = Math::Matrix::Identity();
		Math::Matrix _tMat = Math::Matrix::Identity();
		// 拡縮
		if (a_tinyModel.nodes[_nodeIdx].scale.size() != 0)
		{
			_sMat = Math::Matrix::CreateScale(
		...
		if (a_tinyModel.nodes[_nodeIdx].rotation.size() != 0)
		{
			...
			_rMat = Math::Matrix::CreateFromQuaternion(_quat);
		}
		// 移動
		if (a_tinyModel.nodes[_nodeIdx].translation.size() != 0)
		{
			_tMat = Math::Matrix::CreateTranslation(
...
std::vector<int> ParseBoneIndices(const tinygltf::Model& a_tinyModel, std::vector<Engine::Resource::Parse::RawNode>& a_nodes)
{
	...
	if (a_tinyModel.skins.size() > 0)
	{
		...
		for (UINT _jointIdx = 0; _jointIdx < a_tinyModel.skins[0].joints.size(); ++_jointIdx)
		{
			...
			Math::Matrix _invBindMat = Math::Matrix::Identity();
		...
		for (int _nodeIdx : _result)
		{
			...
			{
				Math::Matrix _boneWorldMat(_boneNode->worldTransform);
				Math::Matrix _invBindMat(a_nodes[_boneNode->parent].inverseBindMatrix);
...
void CalcWorldTransform(Engine::Resource::Parse::RawModel& a_model)
{
	...
	// 行列計算用再起関数
	std::function<void(Engine::Resource::Parse::RawNode*, const Math::Matrix*)> _rec =
		[&_rec, &a_model](Engine::Resource::Parse::RawNode* a_node, const Math::Matrix* a_parentMat)
		{
			// ワールド行列をもとめる
			if (a_parentMat)
			{
				Math::Matrix _localMat(a_node->localTransform);
				Math::Matrix _parentMat(*a_parentMat);
	...
	for (UINT _animaIdx = 0; _animaIdx < a_model.animations.size(); ++_animaIdx)
	{
		...
		for (const auto& _channel : _srcAnima.channels)
		{
			...
			if (_channel.target_path == "translation")
			{
				...
				{
					Engine::Resource::AnimationKeyVector3 _vec;
