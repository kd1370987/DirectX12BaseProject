...
bool SceneManager::Init()
{
	...
	for (size_t _i = 0; _i < g_meshes.size(); ++_i)
	{
		...
		auto _stride = sizeof(AssimpVertex);
	...
	for (size_t _i = 0; _i < g_meshes.size(); ++_i)
	{
		...
		auto _handle = g_descriptorHeap->RegisterSRV(_mainTex->Resource());
	...
	g_pipelineState->SetInputLayout(AssimpVertex::InputLayout);
...
void SceneManager::Draw()
{
	...
	for (size_t _i = 0; _i < g_meshes.size(); ++_i)
	{
		...
		_commandList->SetGraphicsRootDescriptorTable(1, g_materialHandles[_i].handleGPU);	// マテリアルテクスチャをセット
