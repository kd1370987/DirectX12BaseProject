...
void ForwardLightingPass::CreatePass()
{
	...
	Resource::ID _shadowMap = m_pRenderGraph->CreateResource(
		{
			.name = "Shadow",
	...
	Resource::ID _backBuffer = m_pRenderGraph->CreateResource(
		{
			.name = "BackBuffer",
	...
	m_passDesc.readResource.push_back(_shadowMap);
	
	m_passDesc.writeResource.push_back(_backBuffer);
