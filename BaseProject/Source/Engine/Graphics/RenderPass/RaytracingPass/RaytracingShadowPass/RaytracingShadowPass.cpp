...
namespace Engine::Graphics
{
	...
	void AddRaytracingShadowPass(D3D12::PipelineStateManager* a_pPSOManager, RenderPassRegistry* a_pRegistry, const EDrawPhase& a_phase)
	{
		...
		_node.executeFunc = [_spPassData](GraphicsEngine* a_pGE, RenderContext* a_pCtx, uint8_t a_passIndex)
		{
			...
			// UAVをバインド
			a_pCtx->BindUAVBindLess(2, a_pGE->RefRenderGraph()->GetUAVHandle("RayShadow"));

			// GBufferIndex
			GBufferIndex _gbIdx = {};
			_gbIdx.depth = a_pGE->RefRenderGraph()->GetSRVHandle("Depth").GetIndex();
			_gbIdx.normal = a_pGE->RefRenderGraph()->GetSRVHandle("GBufferNormal").GetIndex();
			a_pCtx->BindCB()->BindAndAttachDataComputeRootCBV<GBufferIndex>(
				_pCmdList,
				3,
				_gbIdx
