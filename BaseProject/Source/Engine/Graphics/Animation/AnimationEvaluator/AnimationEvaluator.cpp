...
void Engine::Animation::Interpolate(const Engine::Resource::AnimationNode& a_node, float a_currentTime, Math::Matrix& a_rDst)
{
...
bool Engine::Animation::InterpolateTranslations(const Engine::Resource::AnimationNode& a_node, float a_currentTime, Math::Vector3& a_resullt)
{
	...
	else
	{
		...
		// 補間
		a_resullt = Math::Vector3::Lerp(_prev.vec, _next.vec, _f);
	}

	return true;
}

bool Engine::Animation::InterpolateRotations(const Engine::Resource::AnimationNode& a_node, float a_currentTime, Math::Quaternion& a_resullt)
{
	...
	else
	{
		...
		// 補間
		a_resullt = Math::Quaternion::Slerp(_prev.quat, _next.quat, _f);
	}

	return true;
}

bool Engine::Animation::InterpolateScale(const Engine::Resource::AnimationNode& a_node, float a_currentTime, Math::Vector3& a_resullt)
{
...
void Engine::Animation::CalcNodeMatrix(
	...
	Math::Matrix* a_pOutLocalMat,
	Math::Matrix* a_pOutWorldMat
)
{
	...
	{
		a_pOutWorldMat[a_nodeIdx] = a_pOutLocalMat[a_nodeIdx] * a_pOutWorldMat[a_parentNodeIdx];
	}
	else
	{
...
)
{
	...
	if (a_parentNodeIdx >= 0)
	{
		a_nodePoseVec[a_nodeIdx].world = a_nodePoseVec[a_nodeIdx].local * a_nodePoseVec[a_parentNodeIdx].world;
	}
	else
	{
