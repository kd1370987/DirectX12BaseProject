...
public:
	...
	/// <param name="a_pDSVDesc">ビュー設定</param>
	/// <returns>登録したインデックス</returns>
	DSVHandle RegisterDSV(ID3D12Resource* a_resource, D3D12_DEPTH_STENCIL_VIEW_DESC* a_pDSVDesc);
	...
	const D3D12_CPU_DESCRIPTOR_HANDLE GetCPUHandle(const DSVHandle& a_handleIndex) const;
...
protected:
	...
	// 使用インデックスの待ち行列
	std::queue<DSVHandle> m_indexQueue;
};