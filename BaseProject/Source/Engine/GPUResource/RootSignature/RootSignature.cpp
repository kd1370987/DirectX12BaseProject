...
RootSignature::RootSignature()
{
	...
	//定数バッファとテクスチャの2
	CD3DX12_ROOT_PARAMETER _rootParam[2] = {};
	// b0の定数バッファを設定、全てのシェーダーから見えるようにする
	_rootParam[0].InitAsConstantBufferView(0,0,D3D12_SHADER_VISIBILITY_ALL);

	// t0のテクスチャを設定、全てのシェーダーから見えるようにする
	CD3DX12_DESCRIPTOR_RANGE _tableRange[1] = {};					// ディスクリプタレンジの設定（テーブル）
	_tableRange[0].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 0);		// シェーダーリソースビュー
	_rootParam[1].InitAsDescriptorTable(std::size(_tableRange), _tableRange, D3D12_SHADER_VISIBILITY_ALL);
