...
void RenderingEngine::EndRender()
{
	...
	m_pQueue->ExecuteCommandLists(1, _ppCmdLists);
...
// 
//==================================================================================
ID3D12Device6* RenderingEngine::Device()
{
	// デバイスの取得
...
bool RenderingEngine::CreateSwapChain()
{
	...
	_hr = _pFactory->CreateSwapChain(m_pQueue.Get(), &_desc, &_pSwapChain);
	if (FAILED(_hr))
	{
		_pFactory->Release();
		printf("スワップチェインの生成に失敗");
...
bool RenderingEngine::CreateCommandList()
{
	...
	{
		_hr = m_pDevice->CreateCommandAllocator(
	...
	// コマンドリストの生成
	_hr = m_pDevice->CreateCommandList(
		0,
		D3D12_COMMAND_LIST_TYPE_DIRECT,
...
bool RenderingEngine::CreateFence()
{
	...
	auto _hr = m_pDevice->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(m_pFence.ReleaseAndGetAddressOf()));
	if (FAILED(_hr))
	{
		printf("フェンスの作成に失敗");
		return false;
...
bool RenderingEngine::CreateRenderTarget()
{
	...
	// ディスクリプタヒープをGPU上に作成
	auto _hr = m_pDevice->CreateDescriptorHeap(&_desc,IID_PPV_ARGS(m_pRtvHeap.ReleaseAndGetAddressOf()));
	if (FAILED(_hr))
	{
	...
	for (UINT _i = 0; _i < FRAME_BUFFER_COUNT; ++_i)
	{
		...
		m_pDevice->CreateRenderTargetView(m_pRenderTargets[_i].Get(),nullptr,_rtvHandle);
		// 次のRTV用にハンドルをずらす
...
bool RenderingEngine::CreateDepthStencil()
{
	...
	auto _hr = m_pDevice->CreateDescriptorHeap(&_heapDesc, IID_PPV_ARGS(&m_pDsvHeap));
	if (FAILED(_hr))
	{
	...
	// リソースを実際に作成(GPU上に)
	_hr = m_pDevice->CreateCommittedResource(
		&_heapProp,
	...
	m_pDevice->CreateDepthStencilView(m_pDeptchStencilBuffer.Get(), nullptr, _dsvHandle);
...
void RenderingEngine::WaitRender()
{
	...
	m_pQueue->Signal(m_pFence.Get(), _fenceValue);
