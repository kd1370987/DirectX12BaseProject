...
// 変換はここに集め、呼び出し側で成分を並べ替えないこと。
//
// ・Jolt は右手系/左手系を持たない(重力の向きも設定値)ので、座標はそのまま渡してよい。
//   エンジンの左手系 +Z 前方はそのまま通る。
// ・Quaternion は回転そのものの表現なので、成分(x,y,z,w)をそのまま写す。
// ・Matrix は DirectX と同じ「行ベクトル・行優先(4行目が平行移動)」、
//   Mat44 は「列ベクトル・列優先(4列目が平行移動)」。
//   式としては転置の関係だが、メモリ上の並びは同じなので、
//   こちらの第 i 行をそのまま Mat44 の第 i 列として渡せば同じ変換になる。
// ・JPH_DOUBLE_PRECISION は使っていないので RVec3 == Vec3。
//==========================================================================================
